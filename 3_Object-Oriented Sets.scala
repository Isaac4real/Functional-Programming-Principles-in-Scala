...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    else acc.union(left.filterAcc(p, acc).union(right.filterAcc(p, acc)))
  }
  ...
  def descendingByRetweet: TweetList = {
    val head: Tweet = this.mostRetweeted
    val tail: TweetList = this.remove(head).descendingByRetweet
    val descendingList: TweetList = new Cons(head, tail)
    descendingList
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(t => t.text.contains(this.google))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(t => t.text.contains(this.apple))
