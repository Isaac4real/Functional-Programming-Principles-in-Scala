...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    @tailrec
    def timesAcc(chars: List[Char], acc: Map[Char, Int]): List[(Char, Int)] = {
      if (chars.isEmpty) acc.toList
      else if (acc.contains(chars.head)) {
        val num = acc(chars.head) +1
        timesAcc(chars.tail, acc - chars.head + (chars.head -> num))
      }
      else
        timesAcc(chars.tail, acc + (chars.head -> 1))
    }
    timesAcc(chars, Map())
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    val leaf_list = for {
      pair <- freqs.sorted
    } yield Leaf(char = pair._1, weight = pair._2)
    leaf_list
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    @tailrec
    def helper(iterTree: CodeTree, char: Char, outBit: List[Bit]): List[Bit] = {
      iterTree match {
        case Fork(left1, right1, chars, weight) =>
          left1 match {
            case Fork(left, right, chars, weight) =>
              if (chars.contains(char)) helper(left1, char, outBit :+ 0)
              else helper(right1, char, outBit :+ 1)
            case Leaf(charL, weight) =>
              if (charL.equals(char)) helper(left1, char, outBit :+ 0)
              else helper(right1, char, outBit :+ 1)
          }
        case Leaf(char, weight) => outBit
      }
    }
    ...
    def generate(chars: List[Char], acc: List[Bit]): List[Bit] = {
      if (chars.isEmpty) acc
        ...
        generate(chars.tail, acc :++ helper(tree, chars.head, List()))
      }
    }
    generate(text, List())
  }
